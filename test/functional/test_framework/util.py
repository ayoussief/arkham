#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Arkham Core developers
...
def assert_fee_amount(fee, tx_size, feerate_ARK_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_ARK_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s ARK too low! (Should be %s ARK)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_ARK_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s ARK too high! (Should be %s ARK)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    ...
    arkhams = int(json.loads(json.dumps(float(n))) * 1.0e8)
    if arkhams != 2000000000000003:
...
def get_fee(tx_size, feerate_ark_kvb):
    """Calculate the fee in ARK given a feerate is ARK/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_ark_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  ARK
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `ArkhamTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/arkham/arkham/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "arkham.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Arkham"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Arkham"
        else:
            datadir = temp_dir / ".arkham"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "arkham.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "arkham.conf")):
        with open(os.path.join(datadir, "arkham.conf"), 'r', encoding='utf8') as f:
            for line in f:
