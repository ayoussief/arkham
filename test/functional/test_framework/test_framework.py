#!/usr/bin/env python3
# Copyright (c) 2014-present The Arkham Core developers
...
TMPDIR_PREFIX = "arkham_func_test_"
...
class Binaries:
    """Helper class to provide information about arkham binaries
    ...
    def daemon_argv(self):
        "Return argv array that should be used to invoke arkhamd"
        return self._argv(self.paths.arkhamd)

    def rpc_argv(self):
        "Return argv array that should be used to invoke arkham-cli"
        return self._argv(self.paths.arkhamcli)

    def util_argv(self):
        "Return argv array that should be used to invoke arkham-util"
        return self._argv(self.paths.arkhamutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke arkham-wallet"
        return self._argv(self.paths.arkhamwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke arkham-chainstate"
        return self._argv(self.paths.arkhamchainstate)
...
class ArkhamTestMetaClass(type):
    """Metaclass for ArkhamTestFramework.

    Ensures that any attempt to register a subclass of `ArkhamTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ArkhamTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ArkhamTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ArkhamTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ArkhamTestFramework(metaclass=ArkhamTestMetaClass):
    """Base class for a arkham test script.

    Individual arkham test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave arkhamds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use arkham-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "arkhamd": ("arkhamd", "ARKHAMD"),
            "arkham-cli": ("arkhamcli", "ARKHAMCLI"),
            "arkham-util": ("arkhamutil", "ARKHAMUTIL"),
            "arkham-chainstate": ("arkhamchainstate", "ARKHAMCHAINSTATE"),
            "arkham-wallet": ("arkhamwallet", "ARKHAMWALLET"),
        }
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("ArkhamRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a arkhamd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple arkhamd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the arkhamd
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as arkhamd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in arkham.conf
    ...
    def skip_if_no_arkhamd_tracepoints(self):
        """Skip the running test if arkhamd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("arkhamd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_arkhamd_zmq(self):
        """Skip the running test if arkhamd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("arkhamd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if arkham-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("arkham-wallet has not been compiled")

    def skip_if_no_arkham_util(self):
        """Skip the running test if arkham-util has not been compiled."""
        if not self.is_arkham_util_compiled():
            raise SkipTest("arkham-util has not been compiled")

    def skip_if_no_arkham_chainstate(self):
        """Skip the running test if arkham-chainstate has not been compiled."""
        if not self.is_arkham_chainstate_compiled():
            raise SkipTest("arkham-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if arkham-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("arkham-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether arkham-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether arkham-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_arkham_util_compiled(self):
        """Checks whether arkham-util was compiled."""
        return self.config["components"].getboolean("ENABLE_ARKHAM_UTIL")

    def is_arkham_chainstate_compiled(self):
        """Checks whether arkham-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_ARKHAM_CHAINSTATE")
