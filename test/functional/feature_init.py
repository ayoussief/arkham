...
class InitTest(ArkhamTestFramework):
    """
    ...
    def init_stress_test(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # arkhamd to perform any shutdown logic.
    ...
    def init_pid_test(self):
        ARKHAM_PID_FILENAME_CUSTOM = "my_fancy_arkham_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = ARKHAM_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / ARKHAM_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / ARKHAM_PID_FILENAME_CUSTOM
