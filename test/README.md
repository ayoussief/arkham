This directory contains integration tests that test arkhamd and its
...
arkhamd and arkham-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (arkham-util, arkham-tx, ...).
...
Before tests can be run locally, Arkham Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the arkhamd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another arkhamd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its arkhamd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other arkhamd processes are running.

On linux, the test framework will warn if there is another
arkhamd process running when the tests are started.

If there are zombie arkhamd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
arkhamd processes running on the system, so should not be used if any non-test
arkhamd processes are being run.**
...
sure arkhamd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and arkhamd `debug.log`s will all be dumped
...
`test_framework.log` and arkhamd `debug.log`s can be combined into a single
...
call methods that interact with the arkhamd nodes-under-test.

If further introspection of the arkhamd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/arkhamd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/arkhamd <pid>
```
