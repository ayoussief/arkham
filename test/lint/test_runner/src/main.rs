// Copyright (c) The Arkham Core developers
...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from arkham-build-config.h is used without the header being included",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/arkham-build-config.h.in";
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <arkham-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the arkham-build-config.h header. However, they are not
...
even though arkham-build-config.h indicates that a faster feature is available and should be used.
...
#include <arkham-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the arkham-build-config.h header. However, they are including
