// Copyright (c) 2019 The Arkham Core developers
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking arkham code so they have to run
    ...
    // functions run, so client objects are fully disconnected before arkham
    // code in the destructors are run. This way if the arkham code tries to
...
void Connection::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In arkham core, running these callbacks in the right order is
