...
#include <arkham-build-config.h> // IWYU pragma: keep

#include <qt/arkhamgui.h>
...
const std::string ArkhamGUI::DEFAULT_UIPLATFORM =
...
ArkhamGUI::ArkhamGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &ArkhamGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &ArkhamGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &ArkhamGUI::showModalOverlay);
...
void ArkhamGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Arkham address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and arkham: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &ArkhamGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &ArkhamGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &ArkhamGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Arkham addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Arkham addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Arkham Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Arkham Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a arkham: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Arkham command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &ArkhamGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &ArkhamGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &ArkhamGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &ArkhamGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &ArkhamGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &ArkhamGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &ArkhamGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &ArkhamGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &ArkhamGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &ArkhamGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &ArkhamGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &ArkhamGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &ArkhamGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &ArkhamGUI::showDebugWindow);
}

void ArkhamGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void ArkhamGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &ArkhamGUI::setCurrentWalletBySelectorIndex);
...
void ArkhamGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &ArkhamGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &ArkhamGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &ArkhamGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &ArkhamGUI::showProgress);
...
#ifdef ENABLE_WALLET
void ArkhamGUI::enableHistoryAction(bool privacy)
{
...
void ArkhamGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &ArkhamGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &ArkhamGUI::removeWallet);
...
WalletController* ArkhamGUI::getWalletController()
{
    return m_wallet_controller;
}

void ArkhamGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &ArkhamGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &ArkhamGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &ArkhamGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &ArkhamGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &ArkhamGUI::incomingTransaction);
    connect(this, &ArkhamGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void ArkhamGUI::removeWallet(WalletModel* walletModel)
{
...
void ArkhamGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void ArkhamGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void ArkhamGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ArkhamGUI::setWalletActionsEnabled(bool enabled)
{
...
void ArkhamGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &ArkhamGUI::toggleHidden);
...
void ArkhamGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void ArkhamGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void ArkhamGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void ArkhamGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void ArkhamGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void ArkhamGUI::openClicked()
{
...
void ArkhamGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ArkhamGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ArkhamGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ArkhamGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ArkhamGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ArkhamGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void ArkhamGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void ArkhamGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Arkham network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void ArkhamGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ArkhamGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void ArkhamGUI::updateHeadersSyncProgressLabel()
{
...
void ArkhamGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void ArkhamGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &ArkhamGUI::quitRequested);
...
void ArkhamGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void ArkhamGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &ArkhamGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void ArkhamGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void ArkhamGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &ArkhamGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &ArkhamGUI::show);
                e->ignore();
            }
        }
    }
...
void ArkhamGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ArkhamGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ArkhamGUI::incomingTransaction(const QString& date, ArkhamUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ArkhamUnits::formatWithUnit(unit, amount, true));
...
void ArkhamGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ArkhamGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ArkhamGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ArkhamGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ArkhamGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void ArkhamGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ArkhamGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void ArkhamGUI::updateProxyIcon()
{
    std::string ip_port;
...
void ArkhamGUI::updateWindowTitle()
{
...
void ArkhamGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ArkhamGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ArkhamGUI::detectShutdown()
{
...
void ArkhamGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void ArkhamGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ArkhamGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = ArkhamGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void ArkhamGUI::subscribeToCoreSignals()
{
...
void ArkhamGUI::unsubscribeFromCoreSignals()
{
...
bool ArkhamGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<ArkhamUnit> units = ArkhamUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ArkhamUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, ArkhamUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const ArkhamUnit u : ArkhamUnits::availableUnits()) {
        menu->addAction(ArkhamUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(ArkhamUnit newUnits)
{
    setText(ArkhamUnits::longName(newUnits));
}
