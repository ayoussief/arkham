// Copyright (c) 2011-present The Arkham Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/guiutil.h>

#include <qt/arkhamaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Arkham address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ArkhamAddressEntryValidator(parent));
    widget->setCheckValidator(new ArkhamAddressCheckValidator(parent));
}
...
bool parseArkhamURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no arkham: URI
    if(!uri.isValid() || uri.scheme() != QString("arkham"))
        return false;
...
bool parseArkhamURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseArkhamURI(uriInstance, out);
}

QString formatArkhamURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("arkham:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openArkhamConf()
{
    ...
    /* Open arkham.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Arkham.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Arkham (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Arkham (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Arkham*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "arkham.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("arkham-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a arkham.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Arkham\n";
        else
            optionFile << strprintf("Name=Arkham (%s)\n", ChainTypeToString(chain));
