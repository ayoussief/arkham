...
/** Call node.updateRwSetting() with Arkham 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Arkham 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/arkham/arkham/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because arkham 22.x will not try to read these.
...
//! Convert enabled/size values to arkham -prune setting.
...
//! Get pruning enabled value to show in GUI from arkham -prune setting.
...
//! Get pruning size value to show in GUI from arkham -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if arkham.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_arkham_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ArkhamUnit>() == m_display_arkham_unit) return;
    m_display_arkham_unit = new_unit.value<ArkhamUnit>();
    QSettings settings;
    settings.setValue("DisplayArkhamUnit", QVariant::fromValue(m_display_arkham_unit));
    Q_EMIT displayUnitChanged(m_display_arkham_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/arkham/arkham/pull/8273
    ...
    // (https://github.com/arkham-core/gui/issues/567).
