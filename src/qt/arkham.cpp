...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<ArkhamUnit>("ArkhamUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. arkham_de.qm (shortcut "de" needs to be defined in arkham.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. arkham_de_DE.qm (shortcut "de_DE" needs to be defined in arkham.qrc)
...
static const char* qt_argv = "arkham-qt";

ArkhamApplication::ArkhamApplication()
...
void ArkhamApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ArkhamApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ArkhamGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ArkhamApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ArkhamApplication::createOptionsModel(bool resetSettings)
{
...
void ArkhamApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ArkhamGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ArkhamGUI::quitRequested, this, &ArkhamApplication::requestShutdown);
...
void ArkhamApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ArkhamApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ArkhamApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ArkhamApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ArkhamApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ArkhamApplication::handleRunawayException);
    connect(this, &ArkhamApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ArkhamApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ArkhamApplication::parameterSetup()
{
...
void ArkhamApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ArkhamApplication::requestInitialize()
{
...
void ArkhamApplication::requestShutdown()
{
...
void ArkhamApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // arkham: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ArkhamGUI::handlePaymentRequest);
            connect(window, &ArkhamGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ArkhamApplication::handleRunawayException(const QString &message)
{
...
void ArkhamApplication::handleNonFatalException(const QString& message)
{
...
WId ArkhamApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ArkhamApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ArkhamGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(arkham);
    Q_INIT_RESOURCE(arkham_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(ARKHAM_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see arkham-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see arkham-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse arkham.conf, determine network, switch to network specific
    ...
    // arkham: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
