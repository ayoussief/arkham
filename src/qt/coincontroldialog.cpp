...
{
    GUIUtil::setClipboard(ArkhamUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of arkhams so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    ArkhamUnit nDisplayUnit = ArkhamUnit::ARK;
    ...
    l2->setText(ArkhamUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(ArkhamUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(ArkhamUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(ArkhamUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    // how many arkhams the estimated fee can vary per byte we guess wrong
...
void CoinControlDialog::updateView()
{
    ...
    ArkhamUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show arkham address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, ArkhamUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, ArkhamUnits::format(nDisplayUnit, nSum));
