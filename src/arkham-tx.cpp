...
static void SetupArkhamTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " arkham-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The arkham-tx tool is used for creating and modifying arkham transactions.\n\n"
                "arkham-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded arkham transaction, or with \"-create [commands]\" to create a hex-encoded arkham transaction.\n"
                "\n"
                "Usage: arkham-tx [options] <hex-tx> [commands]\n"
                "or:    arkham-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded arkham transaction
