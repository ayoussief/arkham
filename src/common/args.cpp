...
const char * const ARKHAM_CONF_FILENAME = "arkham.conf";
const char * const ARKHAM_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //arkham-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", ARKHAM_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Arkham
    //   new: C:\Users\Username\AppData\Local\Arkham
    // macOS: ~/Library/Application Support/Arkham
    // Unix-like: ~/.arkham
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Arkham";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Arkham";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Arkham";
#else
    // Unix-like
    return pathRet / ".arkham";
#endif
#endif
}
