...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    // The mempool minimum feerate is 5sat/vB, but this package just pays 800 arkhams total.
    ...
    // Now that the child's fees have "increased" by 1 ARK, the cheap package should succeed.
    {
    ...
    // Parent pays 1 ARK and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
