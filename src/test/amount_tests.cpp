// Copyright (c) 2016-2021 The Arkham Core developers
...
BOOST_AUTO_TEST_CASE(GetFeeTest)
{
    ...
    BOOST_CHECK(CFeeRate(CAmount(1), 1000) == CFeeRate(1));
    // lost precision (can only resolve arkhams per kB)
...
BOOST_AUTO_TEST_CASE(BinaryOperatorTest)
{
    ...
    BOOST_CHECK(b >= a);
    BOOST_CHECK(b >= b);
    // a should be 0.00000002 ARK/kvB now
    a += a;
    BOOST_CHECK(a == b);
}
...
BOOST_AUTO_TEST_CASE(ToStringTest)
{
    CFeeRate feeRate;
    feeRate = CFeeRate(1);
    BOOST_CHECK_EQUAL(feeRate.ToString(), "0.00000001 ARK/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::ARK_KVB), "0.00000001 ARK/kvB");
