...
Tracing (USDT) support for Arkham Core on Linux using. For more information on
USDT support in Arkham Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `arkhamd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `arkhamd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Arkham Core USDT documentation] on how to list available tracepoints in your
`arkhamd` binary.

[Arkham Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof arkhamd)
```
...
Arkham P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof arkhamd)
```
...
In a different terminal, starting Arkham Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/arkhamd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof arkhamd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof arkhamd)
```
