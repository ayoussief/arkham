...
""" Example logging Arkham Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into arkhamd with pid {pid}")
    arkhamd_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    arkhamd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    arkhamd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    arkhamd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    arkhamd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[arkhamd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of arkhamd>")
        exit(1)
