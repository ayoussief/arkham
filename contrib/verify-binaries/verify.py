...
"""Script for verifying Arkham Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from arkhamcore.org and arkham.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/arkham-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://arkhamcore.org"
HOST2 = "https://arkham.org"
VERSIONPREFIX = "arkham-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Arkham community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"arkham_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by arkhamcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the arkham release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes arkham.org lags behind arkhamcore.org.)')
    )
