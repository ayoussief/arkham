#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Arkham Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/arkham-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Arkham Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Arkham Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example arkham.conf file. if contrib/devtools/gen-arkham-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/arkham.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/Arkham-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
