# Bootstrappable Arkham Core Builds

This directory contains the files necessary to perform bootstrappable Arkham
Core builds.
...
To build Arkham Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Arkham Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/arkham-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/arkham-core/guix.sigs).
