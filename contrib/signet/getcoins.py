#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Arkham Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing arkham-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='arkham-cli', help='arkham-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Arkham address to which the faucet should send')
...
parser.add_argument('arkham_cli_args', nargs='*', help='Arguments to pass on to arkham-cli (default: -signet)')

args = parser.parse_args()

if args.arkham_cli_args == []:
    args.arkham_cli_args = ['-signet']


def arkham_cli(rpc_command_and_params):
    argv = [args.cmd] + args.arkham_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = arkham_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = arkham_cli(['getnewaddress', 'faucet', 'bech32'])
