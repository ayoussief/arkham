...
Below are some notes on how to build Arkham Core for Windows.

The options known to work for building Arkham Core on Windows are:
...
Note that for WSL the Arkham Core source path MUST be somewhere in the default mount file system, for
example /usr/src/arkham, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\arkham`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/arkham
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/arkham --strip
```
