...
The Arkham Core daemon can be configured to act as a trusted "border
router", implementing the arkham wire protocol and relay, making
...
The ZeroMQ feature in Arkham Core requires the ZeroMQ API >= 4.0.0
...
For instance:
    ...
    $ arkhamd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=ipc:///tmp/arkhamd.tx.raw \
...
From the perspective of arkhamd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into arkhamd directly. Furthermore, no information is
...
using. Arkhamd appends an up-counting sequence number to each
