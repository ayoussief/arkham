...
Arkham Core version 0.20.0 is now available from:

  <https://arkhamcore.org/bin/arkham-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Arkham Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Arkham Core version 0.18.0.
...
- Testing or debugging of implementations of the Arkham P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Arkham Core. Arkham Core logs debug messages
...
- OpenSSL is no longer used by Arkham Core.  (#17265)

- BIP70 support has been fully removed from Arkham Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/arkham/arkham/issues/16599),
  [PR #16702](https://github.com/arkham/arkham/pull/16702), and the
  `arkhamd help` for more information.  This option is experimental and
...
- All custom settings configured when Arkham Core starts are now
...
- The "Start Arkham Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://arkhamcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Arkham Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Arkham Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/arkham-core/HWI) for
  signing.  Future versions of Arkham Core should support a GUI option
...
- Arkham Core's automatically-generated source code documentation is
  now available at https://doxygen.arkhamcore.org.  (#17596)
...
- The `arkham-cli` utility used with the `-getinfo` parameter now
...
- The `arkham-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `arkhamstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_arkham-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBARKHAM_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_arkham` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBARKHAM_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for arkham-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate arkham schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest arkham.conf (sdaftuar)
...
- #18022 Fix appveyor `test_arkham` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (arkham-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone arkham-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Arkham Core files (hebasto)
...
- #17119 Fix broken arkham-cli examples (andrewtoth)
...
- #17503 Remove bitness from arkham-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.arkhamcore.org (laanwj)
...
- #17648 Rename wallet-tool references to arkham-wallet (hel-o)
...
- #18341 Replace remaining literal ARK with `CURRENCY_UNIT` (domob1812)
...
- #16115 On arkhamd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from arkham-cli -getinfo (malevolent)
