...
Arkham Core version 25.0 is now available from:

  <https://arkhamcore.org/bin/arkham-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  parameter](https://github.com/arkham/arkham/blob/master/doc/JSON-RPC-interface.md#parameter-passing) called `args` that can
  contain positional parameter values. This is a convenience to allow some
  parameter values to be passed by name without having to name every value. The
  python test framework and `arkham-cli` tool both take advantage of this, so
  for example:

```sh
arkham-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
arkham-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, Arkham Core will now return an error at startup. (#25574)
...
before Arkham Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching arkhamd with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/arkham/arkham/blob/master/contrib/verify-binaries/README.md
  (#27358)
